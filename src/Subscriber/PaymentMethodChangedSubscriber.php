...
class PaymentMethodChangedSubscriber implements EventSubscriberInterface
{
    ...
    /**
     * Check if the payment method is being changed and update the COD fee
     */
    public function onKernelController(ControllerEvent $event): void
    {
        ...
        // Check if this is a payment method change route
        $isPaymentMethodChange = $route === 'frontend.account.payment.save' ||
                                  $route === 'frontend.checkout.configure';

        if (!$isPaymentMethodChange) {
            return;
        }

        // Get SalesChannelContext and Cart
        $session = $request->getSession();
        $token = $session->get('sw-context-token');

        if (!$token) {
            return;
        }

        // Get current context and cart
